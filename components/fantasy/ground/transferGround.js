...
function TransferGround(props){
    const {totalInfo, removePlayer,handlePlayerReverse,saveTeam } = props;
    ...
    return(
        <div>
            <div className = "x-transfer-player-ground">
                ...
                <Grid container className = "x-ground-img" style = {mobile?{height: `${imgHeight}px`}:{height:`${imgHeight-220}px`, paddingTop: "10%"}}>
                    ...
                    <Grid item xs = {4} sm = {4} md = {2}>
                        ...
                        <img src={totalInfo[1][0]?`/img/shirts/${totalInfo[1][7]}/keeper.png`:"/img/shirts/empty.png"} className = "x-player-body" width = {mobile?`${imgHeight/10}`:"40px"}   onClick = {(e)=>removePlayer(e,1)}/>
                        <IconButton className = "x-player-remove-button" style = {mobile?{display: "none"}:null}>
                            <InfoIcon />
                        </IconButton>
                        <div className = "x-transfer-player-info-name" style = {mobile?{fontSize: "9px"}:{fontSize: "11px"}}>
            ...
            <div className = "mt-3">
                <button className = "x-button6" onClick = {saveTeam}>Make Transfer</button>
            </div>
        </div>
    )
}
